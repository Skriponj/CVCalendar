...
// MARK: first and last day views in month
extension CVCalendarWeekView {
    public func firstMonthDay() -> CVCalendarDayView {
        var firstMonthDay: CVCalendarDayView!
        if !(self.dayViews.first?.isOut)! {
            firstMonthDay = self.dayViews.first!
            return firstMonthDay
        }
        for (index, dayView) in self.dayViews.enumerated() {
            if !(index + 1 >= self.dayViews.count) {
                if dayView.isOut && !self.dayViews[index + 1].isOut {
                    firstMonthDay = self.dayViews[index + 1]
                }
            }
        }
        return firstMonthDay
    ...
    public func lastMonthDay() -> CVCalendarDayView {
        var lastMonthDay: CVCalendarDayView!
        if !(self.dayViews.last?.isOut)! {
            lastMonthDay = self.dayViews.last!
            return lastMonthDay
        }
        for (index, dayView) in self.dayViews.enumerated() {
            if !(index + 1 >= self.dayViews.count) {
                if !dayView.isOut && self.dayViews[index + 1].isOut {
                    lastMonthDay = dayView
                }
            }
        }
        return lastMonthDay
